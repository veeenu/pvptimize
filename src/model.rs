  ...
  fn try_from(s: &'a gm::PvPMove) -> Result<Self, Self::Error> {
    match s {
      gm::PvPMove::FastMove {
        ...
        type_: Type::try_from(type_.as_str()).map_err(|e| {
      ...
      gm::PvPMove::ChargedMove { unique_id, .. } => Err(Error::ConversionError(format!(
...
impl<'a> TryFrom<&'a gm::PvPMove> for ChargedMove<'a> {
  ...
  fn try_from(s: &'a gm::PvPMove) -> Result<Self, Self::Error> {
    match s {
      ...
      gm::PvPMove::FastMove { unique_id, .. } => Err(Error::ConversionError(format!(
          ...
          .filter_map(|i| match &i.entry {
            Some(gm::GameMasterEntry::PvPMove(m)) => match m {
              gm::PvPMove::FastMove { unique_id, .. } => {
                Some((unique_id.as_str(), FastMove::try_from(m).unwrap()))
              }
              _ => None,
            },
            _ => None,
          ...
          .filter_map(|i| match &i.entry {
            Some(gm::GameMasterEntry::PvPMove(m)) => match m {
              gm::PvPMove::ChargedMove { unique_id, .. } => {
                Some((unique_id.as_str(), ChargedMove::try_from(m).unwrap()))
              }
              _ => None,
            },
            _ => None,
